...
その端で急に曲げられるため、実際にかかっている張力が重りの重さ\SI{230}{g}に比べて
...
修繕後のスパースワイヤーグリッドのたわみ量の評価結果を図\ref{fig:wiresag_swg_result_after}(\subref{fig:wiresag_swg_sag_result_after})に、
たわみ角の評価結果を図\ref{fig:wiresag_swg_result_repair}(\subref{fig:wiresag_swg_sag_angle_result_repair})に示す。
また、図\ref{fig:wiresag_swg_result_repair}(\subref{fig:wiresag_swg_sag_result_repair})に、修繕前の評価結果との比較を示す。
...
\begin{figure}[H]
    ...
    \end{minipage}
    \caption{(\subref{fig:wiresag_swg_sag_result_after}) 修繕後のスパースワイヤーグリッドのたわみ量の評価結果。\ 
             (\subref{fig:wiresag_swg_sag_angle_result_after}) 修繕後のスパースワイヤーグリッドのたわみ量を角度に換算した結果。}
