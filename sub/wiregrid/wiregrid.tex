...
\begin{figure}[H]
    ...
    \colortext{blue}{初見で分かりにくい図な気がしている。もう少しまともな図を作りたい。}
    }
...
この研究結果を受け、新たに Sherborne 社製の DSIC-2051-60 という重力参照計の導入を予定している。
...
本項では、先行研究\cite{swg:murata}にて行われているワイヤーのたわみ量の評価について述べる。

先行研究ではスパースワイヤーグリッドを重力に対して垂直な水平面上に起き、その上にストレートエッジと呼ばれる

ワイヤーは $\SI{230}{g}$ の重りをつけて張力をかけており、

\subsection{系統誤差のまとめと本論文の位置付け}
表\ref{}に、先行研究において調べられた系統誤差の結果をまとめる。
