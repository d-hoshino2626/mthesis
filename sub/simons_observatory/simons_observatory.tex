...
現在、口径$\SI{0.5}{m}$の小口径望遠鏡(Small Aperture Telescope, SAT)\ 3台と、
...
例えば、SATではインフレーションに由来する原始重力波の検出を目指し、その強度指標である
テンソルスカラー比$r$に対して$\sigma(r)=0.003$の精度
\footnote{ここで、$\sigma(X)$は値$X$のエラーバー(ガウス分布の1$\sigma$)を表す。
          当該分野の論文でしばしば用いられる表記であり、本論文でもこれを踏襲する。}
で行う。
また、LATではニュートリノの有効世代数$N_\mathrm{eff}$を$\sigma(N_{\mathrm{eff}})=0.07$、
...
その一方で、大口径の望遠鏡は一度に観測できる視野範囲も小さくなるため、大角度の相関を観測するのに時間を要し、時間変動による大気揺らぎの影響を受けやすくなってしまう。
...
3台のSATのうち2台は$\SI{93}{GHz}$、$\SI{145}{GHz}$帯(Middle Frequency, MF帯)での観測を行い、
1台は$\SI{225}{GHz}$、$\SI{280}{GHz}$帯(Ultra High Frequency, UHF帯)での観測を行う。
また、日本グループが主導して追加される1台のSATは$\SI{27}{GHz}$、$\SI{39}{GHz}$帯(Low Frequency, LF帯)での観測を行う。
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{simons_observatory/so_noise_freq.png}
    \caption{SATの観測帯域と、CMB偏光信号、ダスト放射、シンクロトロン放射の輝度温度の周波数依存性の比較。}
    \label{fig:so_noise_freq}
\end{figure}

図\ref{fig:so-sat_baffle}にSATの概観を示す。
フォアバッフル・コムービングシールド・グラウンドスクリーンと呼ばれる3つのバッフル群を用いて地面からの熱放射や、
近くの山からの照り返しを遮蔽することで、それらが受信機に混入することで生じる外来ノイズを抑えている\cite{Kiuchi_2020}。
...
SATはelevationと呼ばれる仰角方向の角度と、azimuthと呼ばれる方位角方向の角度を回転する機能に加えて、
boresightと呼ばれる視線方向に対する回転も可能である。
\begin{figure}[H]
    \centering
...
CMBをはじめとする空からの光信号は窓を透過後、後述する極低温連続回転式半波長板(HWP)を通過する際に
変調されたのち、光学筒で集光されて焦点面検出器に到達する。
光学筒には3枚のシリコンレンズが搭載されている。
焦点面にはTESボロメータが搭載されており、受信機に斜めに挿さっている希釈冷凍機システムによって
$\SI{100}{mK}$にまで冷却・維持する\cite{galitzki2024simonsobservatorydesignintegration}。

望遠鏡の窓の前面には、本論文にて扱うスパースワイヤーグリッドを用いた偏光角較正装置が取り付けられている。
望遠鏡のあらゆる光学系の目前(信号の入射経路の上流)に設置されているため、
望遠鏡の窓から焦点面に至るまでのすべての光学系の不完全性によって生まれる系統的なバイアスを丸ごと較正することができる。
...
SATでは、この大気による熱放射を取り除くために、偏光信号を変調する装置「極低温連続回転式半波長板」
(cryogenic continuously rotating Half-Wave Plate, 以後、単にHWPと呼ぶ)を用いる\cite{so:hwp_yamada}。

HWPは複屈折の特性を持つ素材からなり、素子中のある決まった軸(光軸)に対して電場成分を反転させる。
...
HWPを通過した後の偏光軸の向きは$-\chi$となり、偏光の向きが1軸対称に反転、つまり$-2\chi$だけ変化する(図\ref{fig:so-hwp_satoru})。
この性質により、ストークスパラメータ(定義は付録\ref{chap:stokes}を参照)が
それぞれ$I_{\mathrm{in}}(t), Q_{\mathrm{in}}(t), U_{\mathrm{in}}(t)$であるような直線偏光した入射光を、
HWPを通過したあとに1軸方向に感度を持つ片偏波のアンテナで電場強度を測った際の出力信号$d_m(t)$は
\begin{equation}
    d_{\mathrm{m}}(t) = I_{\mathrm{in}}(t) + \eta\Re\qty[\qty(Q_{\mathrm{in}}(t)+iU_{\mathrm{in}}(t))\exp(-i 4\chi)]
\end{equation}
となる。ここで、$\eta$ は変調効率であり、$\eta\sim1$である。つまり、SOのように、HWPを$\SI{2}{Hz}$で回転させると、連続的に入射する直線偏光の信号を$\SI{8}{Hz}$に変調して出力することになる。
HWPの角振動数を$\omega_{\mathrm{HWP}}$とし、初期位相を$\chi_0$とすると、$\chi(t) = \omega_{\mathrm{HWP}}t + \chi_{0}$と表され、出力信号は
\begin{equation}
    d_{\mathrm{m}}(t) = I_{\mathrm{in}}(t) + \eta\Re\qty[\qty(Q_{\mathrm{in}}(t)+iU_{\mathrm{in}}(t))\exp(-i \qty(4\omega_{\mathrm{HWP}}t + 4\chi_0))]
\end{equation}
となる。検出器はある偏光角方向$\theta_{\mathrm{det}}$にのみ感度を持つため、最終的に検出器が読み出す信号$d_{\mathrm{m}, \mathrm{det}}$は
\begin{equation}
    \label{eq:so-hwp_modulation}
    d_{\mathrm{m}, \mathrm{det}}(t) = I_{\mathrm{in}}(t) + \eta\Re\qty[\qty(Q_{\mathrm{in}}(t)+iU_{\mathrm{in}}(t))\exp\qty{-i \qty(4\omega_{\mathrm{HWP}}t + 4\chi_0 - 2\theta_{\mathrm{det}})}]
\end{equation}
...
$Q_{\mathrm{in}}+iU_{\mathrm{in}}$を得るためには、データ解析の際に$+4\omega_{\mathrm{HWP}}$の
まわりのみを通すバンドパスフィルタ$\mathcal{F}^{\mathrm{BPF}}$を通した後、2倍して位相を元に戻せば良い。
つまり、データを復調処理した後に得られる信号$d_{\mathrm{d, det}}$は
\begin{align}
    d_{\mathrm{d, det}}(t) &= \mathcal{F}^{\mathrm{BPF}}\qty[d_{\mathrm{m,det}}(t)]\times 2\exp\qty{i\qty(4\omega_{\mathrm{HWP}}t + 4\chi_0)} \\
    &= \eta\qty[Q_{\mathrm{in}}(t) + iU_{\mathrm{in}}(t)]\exp\qty[i\qty(2\theta_{\mathrm{det}}+4\chi_0)]
...
$C_{\ell}^{EE}$は$C_{\ell}^{BB}$と比較して$3\sim4$桁も大きな信号であるため、
...
$\delta\theta < 0.1\tcdegree$であればこの漏れ込みが生むテンソルスカラー比$r$への系統誤差は無視できるほどに小さくなる~\cite{so:Bryan_2018}。
以上の理由から、SOでの偏光角較正の要求精度を $\delta\theta < 0.1\tcdegree$ と定める。
...
全検出器を同時に構成することができない。
さらに、SOの拠点であるチリのアタカマ砂漠は南半球に位置しており、TauAの昇る高度が高くないためSATでは検出器の一部でしか観測できない。
...
月の観測はTauAと同様に観測する月の表面上の位置による偏光角の違いが問題となり、ポインティング精度由来の系統誤差を生じる。
また、月は非常に明るいためTES検出器のダイナミックレンジを超えてしまい、SATでは観測が困難である。
\subsubsection{誘電体シート}
誘電体シートとは、膜状に張られたポリマーがよって環境熱放射を反射させ、直線偏光を作り出す装置である。
...
しかし、$C_{\ell}^{EB}=0$を仮定してしまうため、標準宇宙モデルを仮定したデータ解析となってしまう。
別の言い方をすれば、標準宇宙モデルを超えるような物理に対する感度を捨てる可能性がある。
そのため、この自己較正以外の手法を用いて絶対角度較正を行うことが重要である。
...
\begin{table}[H]
    ...
    \begin{threeparttable}[H]
        ...
        \end{tabular}
        \begin{tablenotes}
            \item[*] 標準宇宙モデル$(C_{\ell}^{EB}=0)$を仮定した手法であり、究極の較正手段とはなり得ない。
        \end{tablenotes}
    \end{threeparttable}
\end{table}
\end{document}