...
それでも偏光角の誤較正は $C_{l}^{EE}$ から $C_{l}^{BB}$ への漏れ込みを引き起こす:
\begin{equation}
...
QUaD実験にて、$0.5\tcdegree$ の絶対角度較正精度が保証された下で $\SI{90,\,150}{GHZ}$観測され、その偏光角が測定されている。~\cite{so:Zemcov_2010}
}
\subsubsection{月}
...
\begin{table}[H]
    \centering
    \caption[先行研究における偏光角較正手法とその精度の比較]{先行研究における偏光角較正手法とその精度の比較}
    \begin{tabular}{cccc}
