...
目盛間は$1.00\pm\SI{0.15}{mm}$であるため、1 pixelが対応する長さは$\SI{5.78}{\mu m}\,\pm\,\SI{0.87}{\mu m}$となった。
...
\end{align}
であった。
スケーラのfittingによれば、1 pixelが対応する長さおよそ$\SI{5.78}{\mu m}$であった。
...
\section{スパースワイヤーグリッドのたわみ量の評価}
\subsection{評価手法}
開発した装置を用いて、実際に使用されるスパースワイヤーグリッドのたわみ量を評価する。
...
なお、アクチュエータの長さの都合上、一度に測定できるのはスパースワイヤーグリッドの半面のみである。
そのため、ワイヤー番号が$0\sim19$のワイヤーと$20\sim38$のワイヤーに分けて測定を行った。
\begin{figure}
    \centering
    \includegraphics[width=0.7\textwidth]{wiresag/wiresag_sparse_wiregrid.pdf}
    \caption{スパースワイヤーグリッドのたわみ量の評価の様子}
    \label{fig:wiresag_sparse_wiregrid}
\end{figure}
\subsection{評価結果とその考察}
図\ref{fig:wiresag_sparse_wiregrid_result}にスパースワイヤーグリッドのたわみ量の評価結果を示す。
