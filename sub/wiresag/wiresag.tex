...
使用したストレートエッジは過去のものと同じく大西測定株式会社製の 140-1000B であり、このストレートエッジは真直度A級 $\SI{30}{\micro m}$ が保証されている。
...
この結果から、得られた評価値と理論値の差は最大で$\SI{50}{\micro m}$程度であるとわかった。
そのため、この評価系で評価されるたわみ量の精度は、$\SI{50}{\micro m}$程度であると結論づける。
\begin{figure}[H]
    \centering
...
理論的に生じるたわみ量との差を見ることによって、およそ$\SI{50}{\micro m}$程度であると見積もられた。
