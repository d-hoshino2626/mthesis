...
図\ref{}、図\ref{}に、$x_{\mathrm{pix}}=\SI{1575}{pixel}$ における輝度を示す。
図\ref{}において、$x_{\mathrm{pix}}<600$ あたりの輝度が低いところはスケーラの目盛の部分であり、
$600<x_{\mathrm{pix}}<1100$ あたりの輝度が高くなっている部分がストレートエッジの表面を、
その後再び急激に低くなるところがストレートエッジの下端を示している。
また、
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{wiresag/wiresag_edge_target.pdf}
    \caption{写真中における $x_{\mathrm{pix}}=\SI{1575}{pixel}$ の目安}
    \label{fig:wiresag_edge_target}    
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{wiresag/wiresag_edge_brightness.pdf}
