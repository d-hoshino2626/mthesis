...
\begin{figure}[H]
    ...
    \caption{$x_{\mathrm{pix}}=\SI{1575}{pixel}$ における輝度の例}
...
\begin{figure}[H]
    ...
    \caption{$x_{\mathrm{pix}}=\SI{1575}{pixel}$ におけるストレートエッジの輝度のfittingの結果の例}
...
\begin{figure}[H]
    ...
    \caption{$x_{\mathrm{pix}}=\SI{1575}{pixel}$ におけるワイヤーの輝度のfittingの結果の例}
...
\begin{align}
    L_{\mathrm{frame}} &= \sqrt{X^2+Y^2}
\end{align}
という拘束条件がある。また、$z$ は距離を表しているので、fittingを行う時には
\begin{align}
...
今回の例では
\begin{align}
    d_{\mathrm{sag}} &= 38.2 \pm \SI{1}{\mu m} \\
    \theta_{\mathrm{sag}} &= 0.011\tcdegree \pm 0.0003 \tcdegree
\end{align}
を得た。
\begin{figure}[H]
    \centering
...
\subsection{評価手法}
これまで述べてきた手法の原理検証として、張力を変えながらのたわみ量を測定する。
...
図\ref{fig:wiresag_performance_check_result}に、張力を変えた時のたわみ量の変化を示す。
横軸は吊るした重りの重さ、縦軸はワイヤーのたわみ量を示している。
青点は評価されたたわみ量を表し、オレンジの線はワイヤーの長さと重りの重さを決めることで
式\eqref{eq:wiresag_catenary_nominal}より求められる理論的なたわみ量を表している。
また、参考としてスパースワイヤーグリッドのワイヤーを張る時に使う重りの重さ$\SI{230}{g}$のところに点線を入れている。
この結果から、スパースワイヤーグリッドにかかる張力付近でのたわみ量を$\order{\SI{10}{\mu m}}$の程度の精度で測定できることがわかった。
\begin{figure}[H]
    \centering
