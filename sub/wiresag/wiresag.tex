...
図\ref{fig:wiresag_picture}から見て取れるように、ストレートエッジは写真中において水平になっていない。
$z$ を求める際には、画像全体を回転し、ストレートエッジが水平になるようにする。
この原因として、ストレートエッジ自体が重力に垂直な水平面上に置かれていないことと、カメラ自体が回転していることが考えられる。
前者に関しては、ストレートエッジが $\SI{1000}{mm}$ の長さに対して両端の高さのずれは多くても $\SI{10}{mm}$ 程度であることから、
