...
\begin{figure}[H]
    ...
    \caption{$x_{\mathrm{pix}}=\SI{1575}{pixel}$ ストレートエッジの輝度のfittingの結果}
    \label{fig:wiresag_edge_fit}
\end{figure}

図\ref{fig:wiresag_picture}から見て取れるように、ストレートエッジは写真中において水平になっていないが、
これはカメラ自体が回転しており、その水平面がストレートエッジと一致していないことが原因である。
この効果を消すためにストレートエッジが画像内で水平になるように画像を回転させる。
こうして得られた画像にて、ストレートエッジに垂直な方向にワイヤーまでの距離を測り、 $z$ として算出する。
図\ref{fig:wiresag_rotation}に、回転前後のストレートエッジとワイヤーの概念図を示す。
$\theta_{\mathrm{camera}}$ はカメラの回転により重力方向が変わる角度であり、
$\theta_{\mathrm{SE}}$ はストレートエッジが傾いているために重力方向が変わる角度である。
この回転はストレートエッジが回転している効果を消去しないので、この効果により
測定された $z$ と本来測るべき $z_{\mathrm{true}}$ との間には誤差が生じる。
その誤差について考える。
$z$ と $z_{\mathrm{true}}$ の関係は
\begin{align}
    z &= z_{\mathrm{true}}\cos\theta_{\mathrm{SE}} \\
    &\simeq z_{\mathrm{true}}\qty(1-\dfrac{\theta_{\mathrm{SE}}^2}{2})
\end{align}
と表される(図\ref{fig:wiresag_rotation})。
...
ストレートエッジの下端の fitting を $1132 < x_{\mathrm{pix}} < 2132$、すなわち、中心から左右 $\SI{500}{pixel}$ の範囲に対して行う。
これによって得られたストレート
