...
そのため、恒温槽を用いてこの温度の下で出力結果を測定し、温度変動による出力の変動を評価を行った。
...
以上の測定系を恒温槽\footnote{恒温槽は京都市産業技術研究所より拝借した。}に入れ、
$\SI{20}{\degreeCelsius}$ から $\SI{-15}{\degreeCelsius}$ まで冷却した。
その後、再び $\SI{20}{\degreeCelsius}$ まで昇温し、出力の変化を測定した。

図\ref{fig:bath_temperature_change}に恒温槽の温度変化の様子を示す。
各温度はおよそ $\SI{3}{時間}$ ほどかけて変化させ、温度が安定している状態が生まれるようにした。
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{tiltsensor/bath_temperature.pdf}
    \figcaption{恒温槽の温度変化}
    \label{fig:bath_temperature_change}
\end{figure}
\subsection{評価結果}
\subsection{評価結果の考察}
\subsection{まとめ}
